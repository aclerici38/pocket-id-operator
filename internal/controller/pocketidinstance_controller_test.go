...
var _ = Describe("PocketIDInstance Controller", func() {
	...
	Context("When creating a PocketIDInstance with Deployment type", func() {
		...
		It("Should create a Deployment", func() {
			...
			Expect(deployment.Spec.Template.Spec.Containers[0].Image).To(Equal("ghcr.io/pocket-id/pocket-id:v2.2.0"))
	...
	Context("When updating a PocketIDInstance", func() {
		...
		It("Should update Deployment when image changes", func() {
			...
			Eventually(func() error {
				...
				instance.Spec.Image = "ghcr.io/pocket-id/pocket-id:v2.2.0"
			...
			}, timeout, interval).Should(Equal("ghcr.io/pocket-id/pocket-id:v2.2.0"))
		})
