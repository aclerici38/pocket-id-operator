...
// +kubebuilder:validation:XValidation:rule="has(self.userID) == has(oldSelf.userID) && (!has(self.userID) || self.userID == oldSelf.userID)",message="userID is immutable"
type PocketIDUserSpec struct {
	...
	// UserID is the optional user ID to use instead of one generated by Pocket-ID.
	// Pocket-ID only accepts a lowercase UUID here, and the ID is immutable once the
	// user is created.
